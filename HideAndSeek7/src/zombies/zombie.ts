...
const zombieDefaults = { moveSpeed: 1, attackDistance: 2, attackTimer: 1, attackTime: 0.5, engageDistance: 16 }
...
export class Zombie extends Character {
  ...
  constructor(transform: TransformType, serverId: string) {
    super(Math.random() > 0.5 ? 'models/ghost.glb' : 'models/BlueGhost.glb', transform)
    this.health = 100
  ...
  createHealthBar() {
    ...
    Transform.create(healthBar, {
      ...
      scale: Vector3.create(utils.remap(this.health, 0, 100, 0, 0.5), 0.05, 0.05),
      parent: this.entity
    })
    MeshRenderer.setBox(healthBar)
    Material.setPbrMaterial(healthBar, { albedoColor: Color4.create(1, 0, 0, 0.6) })
  ...
  damage(damage: number) {
    ...
    Transform.getMutable(this.healthBar).scale.x = utils.remap(this.health, 0, 100, 0, 0.5)
